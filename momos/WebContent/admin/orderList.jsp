	...
	<div class="div_tb_product">
		<table border=1 class="tb_product" align=center>
			<tr style="height:30px">
				<td align=center width=100px>No</td>
				<td align=center width=300px>주문번호</td>
				<td align=center width=100px>이름</td>
				<td align=center width=250px>날짜</td>
				<td align=center width=150px>주문 상태</td>
			</tr>
		...
		<c:set var="oruid" value="${fn:substring(date,0,10)}"/>
			<tr class="row_checkbox">
				...
				<td align=center>
				<a href="detail?or_uid=${list.or_uid}">[${list.or_cart_session }]</a>
				</td>
			...
			<tr>
				<td>
					<select name="field" style="width:90px;height:25px">
						<option value='or_cart_session' <c:if test="${field == 'or_cart_session'}">selected</c:if>>주문번호</option>
						<option value='or_name' <c:if test="${field == 'or_name'}">selected</c:if>>이름</option>
					</select>
				</td>
				<td>
